...
- [x] Task 1.1 (TDD): Gateway unit tests (stub state) — `POST .../bandit-campaigns`
...
- [x] Task 1.2 (Green): Implement `create_bandit_campaign` + `stop_bandit_campaign`
...
- [x] Task 2.1 (TDD): Failing tests — `createBanditCampaign` POSTs
...
- [x] Task 2.2 (Green): Implement the two client fns + `BanditCampaignConfigInput`
...
- [x] Task 3.1 (TDD): Pure config-builder test (`buildCampaignConfig`) +
...
- [x] Task 3.2 (Green): Implement `banditCampaignHelpers.ts` (config builder +
...
- [x] Task 4.1: `cargo test -p stitchd-gateway` green; admin `tsc`/`lint`/vitest/
