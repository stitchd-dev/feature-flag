...
<!-- Last refreshed: 2026-06-09 (post clean_cutover_20260609 — clean cutover to final state: single fresh dated V1 baselines per store, single canonical `events` table (events_v2 retired), proto compat-shim removal + tag compaction, dead legacy code removal. Current-state `events`/`flag_evaluation_log` refs corrected; new Implementation Status row added. No migration path / no backward compat.) -->
...
| **Clean Cutover to Final State (clean_cutover_20260609)** — system not live, so a second hard cutover with **no migration path / no backward compatibility**: collapsed all post-2026-05-25 PostgreSQL (9) + ClickHouse (2) incremental migrations into single fresh dated V1 baselines (`20260609000001`) per store (PG baseline verified functionally identical via round-trip `pg_dump` diff); collapsed ClickHouse to a single canonical `events` table (`events_v2` retired) and folded `evaluation_id` into `flag_evaluation_log`; stripped proto/API backward-compat shims with **tag compaction** (removed `context_hash_specs`/`ContextHashSpec`, `TrackEvent`/`EventFiring` reserved tags, `AdminSegment` always-empty `user_list`/`excluded_keys`); removed dead legacy code paths (the pre-cutover `crates/stitchd-db/clickhouse-migrations/` dir, the unused legacy `MfaChallengeRepository`). | ✅ Complete |
...
- **Preview pipeline (Phase 4):** `POST /v1/metrics/{id}/preview` runs the kind-specific ClickHouse query against `events` and returns a zero-filled daily time-series (days clamped to [1, 90]; default 7). Sparkline-ready.
...
- **Attribution model (post-`experimentation_full_20260521`):** first-exposure intent-to-treat (ITT), derived server-side from `flag_evaluation_log`. SDKs are experiment-unaware — they do NOT tag events with `(experiment, iteration, variant)` tuples. Eval-log rows route through `experiment_assignments_mv` into `experiment_assignments`; stats queries JOIN `events` ⨝ `experiment_assignments` on `(env_id, context_type, context_key)` and filter `e.occurred_at >= a.assigned_at` for strict ITT.
...
- **Pagination:** Top-level resource list views use **true keyset cursor pagination** (post-`platform_hardening_20260608` + `feature-flag-cj5`): `?cursor=<opaque>&limit=N` → `{items, next_cursor}`, with Previous/Next navigation (no page numbers). Each owning service's repo runs a `(created_at, id)` keyset query (`WHERE (created_at, id) > cursor … ORDER BY created_at, id LIMIT n+1`, no `COUNT(*) OVER()`) — O(1) per page, concurrent-insert-stable — and returns the opaque `next_cursor` (owned by `stitchd_db::KeysetCursor`); the proto list RPCs carry `cursor`/`limit` + `next_cursor`; the gateway forwards the token via `CursorPage::from_token`. The Admin UI shares `usePaginatedList` (cursor stack for back-nav, URL-synced via `?cursor=`) + a Prev/Next `Pagination` component. Migrated entities: flags, experiments, segments, events, metrics, sdk-keys, org-users, exclusion-groups (8). Experiment-detail **sub-lists** (iterations, exposures) intentionally remain page-based (`PaginationParams`/`PaginatedResponse`) since they back numbered detail views and the exposure-count stat needs the `total` the cursor envelope omits. (This supersedes `domain_boundaries_20260530`'s page-based canonical, per the `product-guidelines.md` cursor mandate.)
...
- **Partition tuning:** `events` and `flag_evaluation_log` use weekly `toMonday()` partitions + TTL
