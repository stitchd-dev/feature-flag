...
- **Cross-experiment interaction analysis:** for experiments that *do* overlap (different groups / ungrouped), `stitchd-stats-service` self-joins `experiment_assignments` on `(env_id, context_type, context_key)` across candidate pairs (excluding same-group pairs), aggregates each shared metric per variant-combination cell **bounded to first-exposure ITT** (`occurred_at ≥ greatest(a.assigned_at, b.assigned_at)`, matching the platform-wide attribution model), and runs a Frequentist two-way interaction significance test — binary (conversion → 2×2 Wald contrast / R×C Pearson chi-square) and continuous (revenue/duration/numeric → two-way ANOVA F-test). Significance is decided after a **Benjamini–Hochberg (FDR 0.05) correction** across the sweep's test family; under-powered cells are flagged `insufficient_data` (estimate/p_value are 0.0 sentinels, never shown as significant). Results land in ClickHouse `experiment_interactions` and surface via `GetExperimentInteractions` → REST → the Admin UI Interactions tab + a Results-tab warning banner. v1 scope: pairwise only; aggregation/conversion metrics (funnel + ratio skipped); Frequentist only.
