...
- **Opaque token:** base64url(JSON(position)). Clients treat it as opaque and never construct it.
- **Implemented as opaque encoded-offset at the gateway edge (phased approach).** The gateway — the sole REST surface — translates the cursor contract over the existing offset-based service RPCs: it decodes `cursor → offset`, calls the owning service's existing `(offset, limit) → (items, total)` RPC, and emits `next_cursor = encode(offset + len)` whenever `offset + len < total` (else `null`). The token payload is `{offset}`. This delivers the guidelines-mandated cursor **contract** end-to-end with **zero proto/repo churn** — the 6 service repos keep their `*_paginated` (`OFFSET` + `COUNT(*) OVER()`) methods. The shared primitives live in `gateway::pagination` (`CursorParams`, `CursorPage<T>`, `encode_cursor`/`decode_cursor`, `CursorParams::offset()`, `CursorPage::from_offset()`).
- **Trade-off accepted:** the Admin UI moves from numbered pages to next/prev navigation (cursors are opaque; no random page-number access), matching the guidelines. Because `offset` is always a multiple of `limit`, paging is exact and gap/duplicate-free under stable data.
- **True keyset internals deferred (tracked optimization).** Converting the repos from `OFFSET` to keyset (`WHERE (sort_key, id) > $cursor … LIMIT n+1`, dropping `COUNT(*) OVER()`) — which buys O(1) deep-page scans and concurrent-insert stability — is a follow-up that swaps the gateway's `{offset}` token payload for a `{sort_key, id}` payload without changing the REST contract. The `CursorPage::from_overfetch(rows, limit, key_of)` primitive is already in place for that path.
