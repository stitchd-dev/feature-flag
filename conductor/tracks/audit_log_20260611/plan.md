...
- [ ] Task 1.1: Add `crates/stitchd-db/migrations/2026061100xxxx_audit_org_id.sql`
      (ADD COLUMN org_id uuid NULL + index (org_id, created_at DESC, id DESC)).
      Apply to dev DB; verify with `\d audit_log`.
- [ ] Task 1.2: Add a keyset `list_audit_log` repo query + extend `AuditLogger::log`
...
- [ ] Task 2.1 (TDD): Pure tests for the route→resource map + action derivation
...
- [ ] Task 2.2 (Green): Implement `audit` module — `PgAuditWriter` (INSERT via
...
- [ ] Task 3.1 (TDD): Gateway tests — `GET /v1/orgs/{org_id}/audit` authz
      (tenant match / is_system), filters, keyset shape (stub or pool-backed
      smoke 200/empty).
      <!-- files: crates/stitchd-gateway/src/routes/audit.rs -->
- [ ] Task 3.2 (Green): `audit::list_audit` handler + `AuditEntryJson` +
...
- [ ] Task 4.1 (TDD): `listAuditLog` client test; AuditLog page source-contract
...
- [ ] Task 4.2 (Green): `listAuditLog` in api.ts; real `AuditLog` page
...
- [ ] Task 5.1: End-to-end check against the live stack — perform a mutation,
