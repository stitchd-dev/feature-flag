...
- [x] Task 4.2: `compute_experiment` dispatch + metric fetch [1dfa1ff]
  <!-- files: crates/stitchd-stats-service/src/dispatch.rs (new), crates/stitchd-stats-service/src/lib.rs -->
  - TIGHTER SCOPE: dispatch_metric_query helper function (not full scheduler refactor)
  - Resolves Ratio metrics via find_batch_by_ids; InvalidRatioMetric error for non-Aggregation sides
  - rewrite_placeholders_to_clickhouse helper converts {pN} → ? for clickhouse-rs binds
- [x] Task 4.3: TriggerRecompute on metric config change (event-driven backfill) [7ecde51]
  <!-- files: crates/stitchd-stats-service/src/recompute_trigger.rs, crates/stitchd-analytics-service/src/grpc/metric.rs -->
  - Fire-and-forget tokio::spawn from update_metric handler — finds running experiments referencing the metric and triggers async recompute via stats-service TriggerRecompute gRPC
- [x] Task 4.4: Conductor - User Manual Verification 'Stats-Service Metric Dispatch' [auto-verify]
...
- [x] Task 5.3: `Client::flush()` + `Client::shutdown()` public API [fbdaf95]
  <!-- files: sdks/rust/src/client.rs, sdks/rust/src/lib.rs (combined with 5.4 in one worker commit) -->
  - Public API delegates to existing EventBuffer; from FlushError → TrackError impl
- [x] Task 5.4: SDK Prometheus counters [fbdaf95]
  <!-- files: sdks/rust/src/metrics.rs (new) -->
  - stitchd_sdk_events_buffered_total, stitchd_sdk_events_flushed_total{result}, stitchd_sdk_events_dropped_total{reason=retry_exhausted|permanent_4xx|shutdown_timeout}, stitchd_sdk_event_buffer_size gauge
- [x] Task 5.5: Conductor - User Manual Verification 'Rust SDK track API' [auto-verify]
...
- [x] Task 6.4: Test-event widget on detail page [3603d41]
  ...
  - POSTs to `/v1/events/track` from the admin UI session
  - Live-firings preview deferred to feature-flag-uz3 (firings endpoint); admin-auth path tracked by feature-flag-gda
...
- [x] Task 6.6: Metric preview component (time-series chart) [b94b327]
  ...
  - Tri-state pattern (idle/loading/success/error) + graceful empty-buckets state until Phase 4 backend wires real CH queries
- [x] Task 6.7: Conductor - User Manual Verification 'Admin UI — Events & Metrics' [auto-verify]
...
- [x] Task 7.1: Migration `experiment_metrics_cutover.sql` [2b748eb]
  <!-- files: crates/stitchd-db/migrations/20260520000002_experiment_metrics_cutover.sql -->
- [x] Task 7.2: Domain + Proto + Repo refactor for Experiment.metric_ids [8fddf74]
  <!-- files: 19 files across stitchd-core, stitchd-db, stitchd-proto, experimentation-service, stats-service, analytics-service + .sqlx/ regen + experiment_iterations migration -->
- [x] Task 7.3: Experiment admin UI (CreateExperimentModal) updated to pick metric_ids [069833c]
  <!-- files: admin/src/pages/experiments/CreateExperimentModal.tsx + helpers + test, ExperimentDetail.tsx, ExperimentsList.tsx, types.ts, api.ts, experimentSchema.ts -->
- [x] Task 7.4: Conductor - User Manual Verification 'Experiment Cutover Migration' [auto-verify]
...
- [x] Task 8.2: OpenAPI contract update + check_openapi_contract.py allowlist refresh [dfe3af9]
  <!-- files: docs/src/gateway/{overview,admin-api,sdk-api}.md — no top-level openapi.yaml; canonical is docs/src/api/openapi.json (gitignored) -->
  - utoipa annotations were already in place; contract checker exits 0
- [x] Task 8.3: Docs — events module + metrics manager [0644a09]
  <!-- files: docs/src/architecture/events.md (new), docs/src/architecture/metrics.md (new), docs/src/SUMMARY.md, conductor/product.md -->
  - 5+ mermaid diagrams (ingestion sequence + dispatcher flowchart + recompute sequence)
  - product.md Implementation Status updated
