...
**Rationale & impact:** O(1) per-page scans + concurrent-insert stability. **REST contract unchanged** (opaque token) ⇒ Admin UI untouched (verified live end-to-end via grpcurl: page through, exactly-once/no-gaps, cursor decodes to `{created_at, id}`, malformed cursor → `invalid_argument`). Verified CI-green: workspace clippy -D warnings, sqlx-check, fmt, docs idempotent, OpenAPI contract 23/120, stitchd-db + 6 services + analytics (156) all pass. `feature-flag-cj5` closed.

**Follow-up (org-users ordering, same day):** the initial keyset re-keyed `org-users` on `(created_at, id)`, which would have changed its list order from alphabetical-by-email to creation-order. Re-keyed on `(email, id)` (new `stitchd_db::EmailKeysetCursor`; email is globally unique ⇒ stable keyset) to **preserve the original alphabetical order** — now NO user-visible ordering changed by the whole migration.
