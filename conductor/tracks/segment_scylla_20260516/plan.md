...
- [x] Task 1: Update `SegmentRepository` trait surface <!-- commit: d28836b -->
...
- [x] Task 2: Write failing tests for `set_list_entries` (Red) <!-- commit: 458c712 -->
  ...
  - [x] Property test: concurrent readers during swap never see mixed state

- [x] Task 3: Implement `set_list_entries` against Scylla (Green) <!-- commit: 7faeadd -->
  <!-- files: crates/stitchd-db/src/scylla/segment.rs -->
  - [x] Read current generation, write `new_gen` (randomized), CAS-flip pointer
  - [x] Prepared-statement caching via ScyllaClient
  - [x] Tests from Task 2 pass

- [x] Task 4: Write failing tests for `add_entries` / `remove_entries` (Red) <!-- commit: 458c712 -->
  <!-- files: crates/stitchd-db/tests/scylla_segment_repository.rs -->
  - [x] Add and remove against current generation
  - [x] Idempotent on duplicate adds, no-op on missing removes

- [x] Task 5: Implement `add_entries` / `remove_entries` (Green) <!-- commit: 7faeadd -->
  <!-- files: crates/stitchd-db/src/scylla/segment.rs -->
  - [x] Resolve current generation, INSERT/DELETE in that partition
  - [x] Tests from Task 4 pass

- [x] Task 6: Write failing tests for membership read paths (Red) <!-- commit: 458c712 -->
  <!-- files: crates/stitchd-db/tests/scylla_segment_membership.rs -->
  - [x] `check_membership` — include / exclude precedence
  - [x] `batch_check_membership` — multiple keys

- [x] Task 7: Implement membership read paths (Green) <!-- commit: 7faeadd -->
  <!-- files: crates/stitchd-db/src/scylla/segment.rs -->
  - [x] Resolve `active_gen` via generations table
  - [x] Point reads via prepared statements
  - [x] Tests from Task 6 pass

- [x] Task 8: Write failing tests for `get_list_segment_summary` (Red) <!-- commit: 458c712 -->
  <!-- files: crates/stitchd-db/tests/scylla_segment_repository.rs -->
  - [x] Returns counts per context_type
  - [x] Returns empty map for never-populated segments

- [x] Task 9: Implement `get_list_segment_summary` (Green) <!-- commit: 7faeadd -->
  <!-- files: crates/stitchd-db/src/scylla/segment.rs -->
  - [x] COUNT(*) queries over active generation partition
