...
use stitchd_core::experimentation::stats::{
    AnalysisType, VariantStats,
    bayesian, frequentist,
    recommendation::{RecommendationInput, recommend, pick_winner},
};
...
fn main() {
    ...
    println!("  CI (lift)          = [{:.4}, {:.4}]", freq.confidence_interval.lower, freq.confidence_interval.upper);
    ...
    println!("  credible interval  = [{:.4}, {:.4}]", bayes.credible_interval.lower, bayes.credible_interval.upper);
    ...
    println!("[Frequentist]  p={:.4}  significant={}", freq2.p_value, freq2.significant);
    println!("[Bayesian]     prob_best={:.4}", bayes2.prob_best);
    println!("[Recommendations]");
    println!("  Frequentist: {:?}", recommend(&RecommendationInput {
        variant_key: "variant_b".into(), is_control: false,
        frequentist: Some(freq2), bayesian: None,
        analysis_type: AnalysisType::Frequentist,
        sample_size: 200, min_sample_size: None,
    }));
    println!("  Bayesian:    {:?}", recommend(&RecommendationInput {
        variant_key: "variant_b".into(), is_control: false,
        frequentist: None, bayesian: Some(bayes2),
        analysis_type: AnalysisType::Bayesian,
        sample_size: 200, min_sample_size: None,
    }));
    ...
    let ctrl3 = VariantStats {
        sample_size: 50, conversions: Some(5),
        mean: None, variance: None, conversion_rate: Some(0.10), percentiles: None,
    };
    let var3 = VariantStats {
        sample_size: 50, conversions: Some(12),
        mean: None, variance: None, conversion_rate: Some(0.24), percentiles: None,
    };
    let freq3 = frequentist::analyze_count(&ctrl3, &var3);
    println!("[Frequentist]  p={:.4}  significant={}", freq3.p_value, freq3.significant);
    println!("[Recommendation with min_sample_size=200]");
    println!("  {:?}", recommend(&RecommendationInput {
        variant_key: "variant_c".into(), is_control: false,
        frequentist: Some(freq3), bayesian: None,
        analysis_type: AnalysisType::Frequentist,
        sample_size: 50, min_sample_size: Some(200),
    }));
    ...
    let ctrl4 = VariantStats {
        sample_size: 500, conversions: None,
        mean: Some(10.00), variance: Some(25.0),
        conversion_rate: None, percentiles: None,
    };
    let var4 = VariantStats {
        sample_size: 500, conversions: None,
        mean: Some(12.50), variance: Some(28.0),
        conversion_rate: None, percentiles: None,
    };
    let freq4 = frequentist::analyze_numeric(&ctrl4, &var4);
    let bayes4 = bayesian::analyze_numeric(&ctrl4, &var4);
    println!("[Frequentist]  p={:.6}  CI=[{:.2}, {:.2}]  significant={}",
        freq4.p_value, freq4.confidence_interval.lower, freq4.confidence_interval.upper, freq4.significant);
    println!("[Bayesian]     prob_best={:.4}  CI=[{:.2}, {:.2}]",
    ...
    println!("  CI of (variant_p95 - control_p95) = [{:.1}ms, {:.1}ms]", freq5.confidence_interval.lower, freq5.confidence_interval.upper);
    ...
    let recs = vec![
        ("control".to_string(),   stitchd_core::experimentation::stats::Recommendation::ControlWins),
        ("variant_a".to_string(), stitchd_core::experimentation::stats::Recommendation::Inconclusive),
        ("variant_b".to_string(), stitchd_core::experimentation::stats::Recommendation::VariantWins("variant_b".into())),
    ];
