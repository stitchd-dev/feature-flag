use std::sync::Arc;
use stitchd_core::{id::{OrganisationId, ProjectId, FlagId, FlagKey}, tenant::{Organisation, Project}, flag::{FlagRecord, FlagValueType, Variant, VariantValue}};
use stitchd_db::{
    FlagRepository, VariantRepository, ProjectRepository, OrganisationRepository,
    repository::pg::{PgAuditLogger, PgOrganisationRepository, PgProjectRepository, PgFlagRepository, PgVariantRepository},
};
...
async fn test_flag_lifecycle(pool: sqlx::PgPool) {
    ...
    var_repo.create(flag.id, &variant).await.expect("Failed to create variant");

    // 3. Find Flag by Key
    let found = repo.find_by_key(&flag.key, project.id).await.expect("Failed to find flag");
    assert_eq!(found.id, flag.id);

    // 4. Find Variants
    let variants = var_repo.find_by_flag(flag.id).await.expect("Failed to find variants");
    ...
    var_repo.update(&to_update).await.expect("Failed to update variant");

    // 6. Delete Variant
    var_repo.delete(to_update.id).await.expect("Failed to delete variant");
