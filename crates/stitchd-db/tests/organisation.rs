...
async fn test_organisation_lifecycle(pool: sqlx::PgPool) {
    ...
    repo.create(&org).await.expect("Failed to create organisation");

    // 2. Find
    let found = repo.find_by_id(org.id).await.expect("Failed to find organisation");
    ...
    let updated = repo.update(&to_update).await.expect("Failed to update organisation");
    assert_eq!(updated.name, "Updated Org");
    assert_eq!(updated.version, 2);

    // 4. Update (Stale Version -> Conflict)
    let stale_err = repo.update(&found).await.unwrap_err();
    assert!(matches!(stale_err, stitchd_db::RepositoryError::VersionConflict { .. }));

    // 5. List
    let all = repo.list_all().await.expect("Failed to list organisations");
    assert!(all.iter().any(|o| o.id == org.id && o.name == "Updated Org"));

    // 6. Soft Delete
    repo.soft_delete(org.id).await.expect("Failed to soft delete");
