...
async fn migrations_apply_in_order() {
    ...
    let ks = format!("stitchd_mig_{}", &uuid::Uuid::new_v4().simple().to_string()[..8]);
    ...
    migrate::run(&client, migrations_dir).await.expect("migrations should apply");
    ...
    let count = rows
        ...
        .expect("rows result")
        .rows_num();
    assert!(count > 0, "segment_list_entries table should exist after migrations");
...
async fn migrations_are_idempotent() {
    ...
    let ks = format!("stitchd_idem_{}", &uuid::Uuid::new_v4().simple().to_string()[..8]);
    let config = test_config_with_keyspace(&ks);
    let client = ScyllaClient::connect(&config).await.expect("connect");

    let migrations_dir = concat!(env!("CARGO_MANIFEST_DIR"), "/scylla-migrations");
    migrate::run(&client, migrations_dir).await.expect("first run");
    migrate::run(&client, migrations_dir).await.expect("second run should be idempotent");
...
async fn applied_versions_are_tracked() {
    ...
    let ks = format!("stitchd_trk_{}", &uuid::Uuid::new_v4().simple().to_string()[..8]);
    let config = test_config_with_keyspace(&ks);
    let client = ScyllaClient::connect(&config).await.expect("connect");
    ...
    let rows = client
        .session()
        .query_unpaged(
            format!("SELECT version FROM {ks}.scylla_migrations"),
            &[],
        )
        .await
    ...
    assert!(count > 0, "scylla_migrations should have at least one row after applying");
