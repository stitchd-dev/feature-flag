...
async fn get_results_falls_back_to_flat_variant_results_when_per_context_type_empty() {
    ...
    // `results_by_context_type[<ct>]` bucket so the new admin UI still has a
    // shape to render.
    ...
    let svc = MockExpService {
        results: ExperimentResults {
            ...
            variant_results: vec![VariantResult {
                variant_key: "control".to_string(),
                participant_count: 7,
                context_type: String::new(), // empty -> defaults to "user"
                ..Default::default()
            }],
            computed_at_ms: 0,
            is_stale: true,
            next_run_at_ms: 0,
            computation_status: "never_computed".to_string(),
            results_by_context_type: vec![],
            bound_target: None,
            pre_period_days: 0,
        },
        ..Default::default()
    ...
    let by_ct = &body["results_by_context_type"];
    assert!(by_ct["user"].is_object());
    assert_eq!(by_ct["user"]["variants"].as_array().unwrap().len(), 1);
}
