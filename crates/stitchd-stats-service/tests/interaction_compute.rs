...
async fn fetch_rows(ch: &Client, exp: Uuid) -> Vec<InteractionResultRow> {
    let sql = format!(
        "SELECT experiment_id_a, experiment_id_b, context_type, metric_key, shared_count, significant
...
async fn significant_interaction_is_written() {
    ...
    let exps = vec![
        meta(exp_a, Uuid::new_v4(), metric, Utc.with_ymd_and_hms(2026, 5, 1, 0, 0, 0).unwrap()),
        meta(exp_b, Uuid::new_v4(), metric, Utc.with_ymd_and_hms(2026, 5, 1, 0, 0, 0).unwrap()),
    ];
    // distinct flags
    let mut exps = exps;
...
async fn same_group_pair_writes_nothing() {
    ...
    let rows = fetch_rows(&ch, exp_a).await;
    assert!(rows.is_empty(), "no interaction row for same-group pair, got {rows:?}");
}
